...
LANG = json.loads(open(PATH + 'lang/' + CONFIG['Lang'] + '.json').read()) # 加载语言文件
...
def save_preference():  # 保存用户资料与设置
    ...
    while PREFERENCE_LOCK:#!/usr/bin/python
# -*- coding: utf-8 -*-
            ...
            if message_list.has_key(str(update.message.reply_to_message.message_id)):   # 如果消息数据存在
                ...
                try:
                    ...
                    if e.message == "Forbidden: bot was blocked by the user":
                        bot.send_message(chat_id=CONFIG['Admin'],text=LANG['blocked_alert'])    # Bot被停用
                    else:
...
# 添加Handle
dispatcher.add_handler(telegram.ext.MessageHandler(telegram.ext.Filters.all & telegram.ext.Filters.private & (~ telegram.ext.Filters.command) & (~ telegram.ext.Filters.status_update), process_msg))   # 处理消息

dispatcher.add_handler(telegram.ext.MessageHandler(telegram.ext.Filters.command & telegram.ext.Filters.private,process_command))    # 处理指令
